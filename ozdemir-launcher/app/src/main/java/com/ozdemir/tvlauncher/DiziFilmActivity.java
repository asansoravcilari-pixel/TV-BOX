...
public class DiziFilmActivity extends Activity {
    ...
    private View build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(42),dp(28),dp(42),dp(30));root.setBackground(bg(new int[]{0xFF100716,0xFF291039,0xFF08060D},24,0));
        ...
        LinearLayout cards=new LinearLayout(this);cards.setOrientation(LinearLayout.HORIZONTAL);cards.setGravity(Gravity.CENTER);cards.addView(card("▶","puhutv","Ücretsiz dizi ve film",()->openWeb("https://puhutv.com/")),lp());cards.addView(card("T","tabii","TRT dizi • film • belgesel",()->openAppOrWeb("com.trt.tabii.android","https://www.tabii.com/tr")),lp());cards.addView(card("P","Plex","Reklam destekli ücretsiz film ve TV",()->openAppOrWeb("com.plexapp.android","https://www.plex.tv/watch-free-tv/")),lp());root.addView(cards,new LinearLayout.LayoutParams(-1,0,1));View first=cards.getChildAt(0);if(first!=null)first.requestFocus();return root;}

    private View card(String icon,String title,String sub,Runnable action){LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setGravity(Gravity.CENTER);box.setPadding(dp(22),dp(18),dp(22),dp(18));box.setFocusable(true);box.setClickable(true);box.setBackground(cardBg(false));TextView i=text(icon,36,true);i.setGravity(Gravity.CENTER);i.setTextColor(0xFFE9C8FF);TextView t=text(title,23,true);t.setGravity(Gravity.CENTER);t.setPadding(0,dp(12),0,0);TextView s=text(sub,12,false);s.setGravity(Gravity.CENTER);s.setTextColor(0xFFBDA8C8);s.setPadding(0,dp(7),0,0);box.addView(i);box.addView(t);box.addView(s);box.setOnClickListener(v->action.run());box.setOnFocusChangeListener((v,f)->{v.animate().scaleX(f?1.05f:1).scaleY(f?1.05f:1).translationY(f?-dp(3):0).setDuration(120).start();v.setBackground(cardBg(f));i.setTextColor(f?Color.WHITE:0xFFE9C8FF);});return box;}
    ...
    private LinearLayout.LayoutParams lp(){LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(250),1);p.setMargins(dp(12),dp(22),dp(12),dp(22));return p;}
