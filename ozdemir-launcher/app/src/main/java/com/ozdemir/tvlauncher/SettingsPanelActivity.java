...
public class SettingsPanelActivity extends Activity {
    private LinearLayout detail; private SharedPreferences prefs; private int themeIndex;
    ...
    private final String[] themeNames={"Modern Salon","Neon City","Uzay / Nebula","Mor Cam"};
    ...
    private View build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.HORIZONTAL);root.setPadding(dp(28),dp(24),dp(28),dp(24));root.setBackground(new GradientDrawable(GradientDrawable.Orientation.TL_BR,themes[themeIndex]));LinearLayout left=new LinearLayout(this);left.setOrientation(LinearLayout.VERTICAL);left.setPadding(dp(12),dp(12),dp(20),dp(12));TextView logo=text("Ö  ÖZDEMİR TV OS",22,true);logo.setTextColor(0xFFF2DBFF);left.addView(logo);TextView sub=text("Ayarlar Merkezi",12,false);sub.setTextColor(0xFFBDA5C8);sub.setPadding(0,0,0,dp(18));left.addView(sub);left.addView(menu("⌁  Ağ","network"));left.addView(menu("◫  Depolama","storage"));left.addView(menu("◷  Zamanlayıcı","timer"));left.addView(menu("✦  Tema","theme"));left.addView(menu("⌨  Klavye","keyboard"));left.addView(menu("◉  Cihaz Hakkında","about"));left.addView(menu("⏻  Güç","power"));root.addView(left,new LinearLayout.LayoutParams(0,-1,.40f));detail=new LinearLayout(this);detail.setOrientation(LinearLayout.VERTICAL);detail.setPadding(dp(30),dp(24),dp(30),dp(24));detail.setBackground(panelBg());root.addView(detail,new LinearLayout.LayoutParams(0,-1,.60f));return root;}
    ...
    private void settings(){title("ÖZDEMİR Ayarlar","TV için sade, tek tasarım ve kumanda odaklı kullanım.");detail.addView(info("Kumandayla soldan kategori seç. Temel ayarlar ÖZDEMİR arayüzünün içinde kalır."));}
    ...
    private void timer(){title("Zamanlayıcı","Uyku süresini seç");int active=prefs.getInt("sleep_minutes",0);detail.addView(info(active>0?"Aktif zamanlayıcı: "+active+" dakika":"Zamanlayıcı kapalı"));detail.addView(action("30 dakika","Uyku zamanlayıcısını ayarla",()->saveTimer(30)));detail.addView(action("60 dakika","Uyku zamanlayıcısını ayarla",()->saveTimer(60)));detail.addView(action("90 dakika","Uyku zamanlayıcısını ayarla",()->saveTimer(90)));detail.addView(action("Kapalı","Zamanlayıcıyı iptal et",()->saveTimer(0)));}
    private void saveTimer(int m){prefs.edit().putInt("sleep_minutes",m).putLong("sleep_set_at",System.currentTimeMillis()).apply();Toast.makeText(this,m==0?"Zamanlayıcı kapatıldı":m+" dakika ayarlandı",Toast.LENGTH_SHORT).show();showPage("timer");}
    private void theme(){title("Tema","4 premium ÖZDEMİR görünümü");for(int i=0;i<4;i++){final int x=i;detail.addView(action(themeNames[i],i==themeIndex?"Şu anda kullanılıyor":"Seç ve uygula",()->{themeIndex=x;prefs.edit().putInt("theme",x).apply();recreate();}));}}
    private void keyboard(){title("Klavye","ÖZDEMİR TV klavyesi");detail.addView(info("ÖZDEMİR TV Klavyesi uygulamanın içinde. İlk kurulumda bir kez etkinleştirilmesi gerekir."));detail.addView(action("Klavye seçiciyi aç","ÖZDEMİR TV Klavyesi'ni seç",()->{try{((InputMethodManager)getSystemService(INPUT_METHOD_SERVICE)).showInputMethodPicker();}catch(Exception e){openSystem(Settings.ACTION_INPUT_METHOD_SETTINGS);}}));}
    private void about(){title("Cihaz Hakkında","ÖZDEMİR TV OS v1.1");detail.addView(info("Android 10 tabanlı TV sistemi\nÖZDEMİR Launcher\nMedia3 canlı TV motoru\nUygulama Arşivi\nDizi & Film merkezi\nÇocuklar merkezi\n4 premium tema\nTV kumandası için optimize edildi"));}
    private void power(){title("Güç","Cihaz güç seçenekleri");detail.addView(action("Ana ekrana dön","ÖZDEMİR TV OS",this::finish));detail.addView(info("Tam yeniden başlatma / kapatma Android sistem yetkisi gerektirir. Cihaz testinde ADB yetkisiyle bağlanacak."));}
