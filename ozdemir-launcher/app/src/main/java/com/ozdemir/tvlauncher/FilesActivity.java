...
public class FilesActivity extends Activity {
 ...
 private String mime(File f){String n=f.getName().toLowerCase();if(n.endsWith(".mp4"))return "video/mp4";if(n.endsWith(".mkv"))return "video/x-matroska";if(n.endsWith(".avi"))return "video/x-msvideo";if(n.endsWith(".jpg")||n.endsWith(".jpeg"))return "image/jpeg";if(n.endsWith(".png"))return "image/png";if(n.endsWith(".webp"))return "image/webp";if(n.endsWith(".mp3"))return "audio/mpeg";if(n.endsWith(".wav"))return "audio/wav";if(n.endsWith(".pdf"))return "application/pdf";if(n.endsWith(".apk"))return "application/vnd.android.package-archive";return "*/*";}
 private void open(File f){try{Uri u=FileProvider.getUriForFile(this,getPackageName()+".files",f);Intent i=new Intent(Intent.ACTION_VIEW);i.setDataAndType(u,mime(f));i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION|Intent.FLAG_ACTIVITY_NEW_TASK);startActivity(i);}catch(Exception e){android.widget.Toast.makeText(this,"Bu dosyayı açacak uygulama bulunamadı.",android.widget.Toast.LENGTH_SHORT).show();}}
